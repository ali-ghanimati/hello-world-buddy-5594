...
export const Route = createFileRoute("/account")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "داشبورد مشتری شیکو: سفارش‌های اخیر، وضعیت ارسال و نشانی‌های ذخیره‌شده." },
      { property: "og:title", content: "حساب کاربری | شیکو" },
      { property: "og:description", content: "سفارش‌ها، وضعیت ارسال و اطلاعات حساب شما." },
  ...
  if (!ready) {
    return (
      <div className="container-shiko py-16">
        <div className="h-40 animate-pulse rounded-md bg-secondary" aria-label="در حال بارگذاری" />
      </div>
    );
  }
    ...
    <div className="container-shiko py-8 sm:py-10">
      ...
      <header className="mt-5 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4">
        <div className="min-w-0">
          <h1 className="truncate text-2xl font-black sm:text-3xl">سلام {user.name}</h1>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[14rem_minmax(0,1fr)]">
        <nav aria-label="بخش‌های حساب" className="overflow-x-auto">
        ...
        <div>
                ...
                {orders.map((order) => (
                  <li key={order.id} className="rounded-md border border-border bg-card p-5">
                    <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
                      <div className="min-w-0">
                        <h2 className="text-sm font-bold">سفارش {order.id}</h2>
                        <p className="mt-1 text-xs text-muted-foreground">ثبت شده در {order.date}</p>
                      </div>
                      <span
                        className={cn(
                    ...
                    <ul className="mt-4 space-y-2 border-t border-border pt-4 text-xs text-muted-foreground">
                      {order.items.map((item, i) => (
                        <li key={i} className="flex flex-wrap justify-between gap-2">
                          <span>
                            {item.name} — سایز {item.size}، {item.color} × {toPersianDigits(item.quantity)}
                          </span>
                    ...
                    <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-border pt-4 text-sm">
                      ...
                      <span className="font-bold">{formatToman(order.total)}</span>
                    </div>
                  </li>
                ))}
              </ul>
          ...
          {tab === "profile" && (
            <div className="rounded-md border border-border bg-card p-5">
              <h2 className="text-sm font-bold">اطلاعات حساب</h2>
              <dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2">
                ...
                <div>
                  <dt className="text-xs text-muted-foreground">موبایل / ایمیل</dt>
                  <dd className="mt-1" dir="ltr">
                    {user.identifier}
                  </dd>
                </div>
                <div>
                  <dt className="text-xs text-muted-foreground">تعداد سفارش‌ها</dt>
                  <dd className="mt-1">{toPersianDigits(orders.length)}</dd>
                </div>
                <div>
                  <dt className="text-xs text-muted-foreground">عضویت از</dt>
                  <dd className="mt-1">خرداد ۱۴۰۳</dd>
                </div>
              </dl>
            </div>
          )}
          ...
          {tab === "addresses" && (
            <ul className="grid gap-4 sm:grid-cols-2">
              {[
                { title: "نشانی منزل", body: "تهران، خیابان ولیعصر، کوچه بهار، پلاک ۱۲، واحد ۳" },
                { title: "نشانی محل کار", body: "تهران، سعادت‌آباد، بلوار دریا، پلاک ۴۵، طبقه ۲" },
              ].map((a) => (
                <li key={a.title} className="rounded-md border border-border bg-card p-5">
